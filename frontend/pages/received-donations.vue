...
<div v-if="donations != null">
  ...
  <table class="table table-auto">
    ...
    <tbody>
      <tr v-for="donation in donations.results" :key="donation.id">
        <td>{{ donation.fan_user.display_name }}</td>
  ...
  <fm-dialog v-if="activeDonation" v-model="showDialog">
    <template #header>{{ activeDonation.fan_user.display_name }}</template>
    <dl>
      <dt>Amount</dt>
      <dd>
...
export default {
  ...
  methods: {
    ...mapActions('donations', ['fetchReceivedDonations', 'fetchMoreDonations', 'fetchPayments', 'fetchMorePayments']),
    loadDonations() {

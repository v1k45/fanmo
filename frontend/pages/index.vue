      ...
      <div class="row items-center justify-center">
        ...
        <template v-if="creators.length">
          ...
          <div class="flex justify-between items-center md:items-start space-y-8 md:space-y-0 md:flex-row flex-col">
            <nuxt-link v-for="creator in creators" :key="creator.username" :to="`/${creator.username}`" class="unstyled transform transition-transform hover:scale-105 md:w-1/4">
              <div class="flex items-center flex-col">
                ...
                <div class="text-2xl text-black font-bold mt-2 text-center">{{ creator.display_name }}</div>
                <div v-if="creator.one_liner" class="text-gray-500 text-center">{{ creator.one_liner }}</div>
              </div>
            </nuxt-link>
          </div>
        </template>

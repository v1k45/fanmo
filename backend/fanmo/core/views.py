...
def page_view(request, username):
    ...
    title = f"{user.display_name} {user.one_liner}"
    description = f"Support {user.display_name} on Fanmo. Become a member, get access to exclusive content, send tips and much more on Fanmo."
    image_url = None
...
def post_view(request, post_slug, post_id):
    ...
    title = f"{post.title} - {post.author_user.display_name}"
    description = f"Support {post.author_user.display_name} on Fanmo. Become a member, get access to exclusive content, send tips and much more on Fanmo."
    keywords = ""
    if post.meta:
        title = post.meta.title or title
        description = post.meta.description or description
        keywords = post.meta.keywords or keywords

    image_url = None
    if post.meta and post.meta.image:
        image_url = build_versatileimagefield_url_set(
            post.meta.image,
            settings.VERSATILEIMAGEFIELD_RENDITION_KEY_SETS["post_image"],
            request,
        )["social"]
    elif post.social_image:
...
def serve_index(*, title=None, description=None, image=None, keywords=None, status=200):
    """
    ...
    if keywords:
        response_text = replace_format(
            response_text,
            'name="keywords" content="%s" data-hid="keywords"',
            keywords,
        )
